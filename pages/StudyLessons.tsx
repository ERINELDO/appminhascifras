...
import { Plus, ArrowLeft, Loader2, X, Save, Target, CheckCircle2, FileText, MoreVertical, Pencil, Trash2, Zap } from 'lucide-react';
import { StudyDiscipline, StudyTopic } from '../types';
...
export const StudyLessons: React.FC<StudyTopicsProps> = ({ selectedDiscipline, onBack }) => {
  ...
  useEffect(() => {
    if (selectedDiscipline) loadTopics();
  ...
  const loadTopics = async () => {
    ...
    setLoading(true);
    try {
      const data = await api.getStudyTopics(selectedDiscipline.id);
      setTopics(data);
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 gap-4">
          {topics.length === 0 ? (
            ...
            <div key={topic.id} className="bg-white border-2 border-slate-300 p-8 rounded-none flex flex-col md:flex-row md:items-center justify-between gap-6 hover:border-indigo-500 transition-all shadow-[6px_6px_0px_0px_rgba(0,0,0,0.04)] group relative">
